...
function snapshotToQuote(entry: PolygonSnapshotEntry): FinnhubQuote {
  return {
    ...
    h: entry.day.h, l: entry.day.l, o: entry.day.o,
    pc: entry.prevDay.c,
...
interface SpeculativeResult {
  score: number; rsRating: number; volumeRatio: number; rsi: number;
  emaStack: boolean; emaCross921: boolean; aboveVwap: boolean; anchoredVwap: number | null;
  ema9: number[]; ema21: number[]; ema50: number[]; rsiSeries: number[];
  signals: MomentumSignal[]; tradeSetup: TradeSetup; ruleBasedCommentary: string; distanceFromEma9Pct: number;
}

function scoreSpeculative(
  bars: OHLCVBar[], benchmarkBars: OHLCVBar[], ticker: string, name: string,
  ...
  const signals: MomentumSignal[] = [
    { name: "atrCompression", label: "ATR Compression", value: atrRatio < 0.75 ? `${Math.round((1 - atrRatio) * 100)}% tighter than 20d ago` : "Volatility not yet contracting", status: atrRatio < 0.60 ? "pass" : atrRatio < 0.85 ? "warn" : "fail" },
    { name: "ema50Recovery", label: "EMA 50 Recovery", value: aboveEma50 ? `${distFromEma50 > 0 ? "+" : ""}${(distFromEma50 * 100).toFixed(1)}% above EMA 50` : `${(distFromEma50 * 100).toFixed(1)}% below EMA 50`, status: (aboveEma50 && nearEma50) ? "pass" : nearEma50 ? "warn" : "fail" },
    { name: "basePattern", label: "Base Tightness", value: `${(baseRange * 100).toFixed(0)}% 20-day range`, status: baseRange < 0.22 ? "pass" : baseRange < 0.30 ? "warn" : "fail" },
    { name: "volumeContraction", label: "Volume", value: `${volumeRatio.toFixed(1)}x average — ${volumeRatio < 1.0 ? "contracting (bullish for base)" : "expanding"}`, status: volumeRatio < 1.0 ? "pass" : volumeRatio < 1.3 ? "warn" : "fail" },
    { name: "rsImproving", label: "RS Improving", value: rsImproving ? `RS ${rsRating} — up from ${rsOld} (4wk ago)` : `RS ${rsRating} — flat/declining`, status: rsImproving ? "pass" : rsRating >= 40 ? "warn" : "fail" },
    { name: "rsi", label: "RSI (14)", value: `${lastRsi.toFixed(0)} — ${lastRsi >= 45 && lastRsi <= 65 ? "accumulation zone" : lastRsi > 65 ? "approaching momentum" : "oversold territory"}`, status: lastRsi >= 45 && lastRsi <= 75 ? "pass" : lastRsi >= 38 ? "warn" : "fail" },
  ];
  ...
  const tradeSetup: TradeSetup = {
    ...
    target, targetPct, stopLoss, stopLossPct, riskReward, stopReason: "EMA 50 or base low",
  };
  ...
  return { score, rsRating, volumeRatio, rsi: lastRsi, emaStack, emaCross921, aboveVwap, anchoredVwap, ema9, ema21, ema50, rsiSeries, signals, tradeSetup, ruleBasedCommentary, distanceFromEma9Pct };
}
...
export async function runSpeculativeScan(limit = 10): Promise<MomentumPick[]> {
  ...
  await Promise.allSettled(
    SPECULATIVE_UNIVERSE.map(async (ticker) => {
      try {
        ...
        if ((benchmarkLastBarTime - tickerLastBarTime) / 86400 > 5) return;
        ...
        const scored = scoreSpeculative(bars, benchmarkBars, ticker, ticker, { benchmarkName: "Russell 2000", maxStopPct: 0.08 });
        ...
        results.push({
          ...
          score: scored.score, rsRating: scored.rsRating, volumeRatio: scored.volumeRatio, rsi: scored.rsi,
          emaStack: scored.emaStack, emaCross921: scored.emaCross921, aboveVwap: scored.aboveVwap,
          sparkline: bars.slice(-30).map((b) => b.close),
          volumeSparkline: bars.slice(-20).map((b) => b.volume),
          avgDailyVolume: Math.round(bars.slice(-21, -1).reduce((s, b) => s + b.volume, 0) / 20),
          tradeSetup: scored.tradeSetup, signals: scored.signals,
          ruleBasedCommentary: scored.ruleBasedCommentary,
          distanceFromEma9Pct: +scored.distanceFromEma9Pct.toFixed(1),
          isSpeculative: true, type: "stock",
        });
      } catch { /* skip */ }
    })
  );
  ...
  await Promise.allSettled(
    top.map(async (pick) => {
      ...
      if (newsVal && newsVal.count > 0) { pick.newsCount = newsVal.count; pick.latestHeadline = newsVal.latest ?? undefined; }
