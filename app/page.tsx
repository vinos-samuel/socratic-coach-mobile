...
import { TrendingUp, Zap, AlertCircle, RefreshCw, FlameKindling, FlaskConical, Star } from "lucide-react";
...
const SCANNER_TABS: { id: Exclude<Tab, "watchlist">; label: string; api: string }[] = [
...
const DISCLAIMERS: Record<Exclude<Tab, "watchlist">, string> = {
...
async function fetchScanner(tab: Exclude<Tab, "watchlist">, forceRefresh = false): Promise<ScannerResponse> {
  const base = SCANNER_TABS.find((t) => t.id === tab)!.api;
...
export default function DashboardPage() {
  ...
  const [watchlistPicks, setWatchlistPicks] = useState<MomentumPick[]>([]);
  ...
  const { data, isLoading, isError, error, refetch, isFetching } = useQuery<ScannerResponse>({
    ...
    queryFn: () => fetchScanner(tab as Exclude<Tab, "watchlist">, forceRefresh),
  ...
  const scanLabel = tab === "stocks" ? "S&P 500 large caps"
    ...
    : tab === "crypto" ? "top crypto pairs (6H)"
    ...
    <div className="min-h-screen bg-[#0d1210]">
      ...
      <header className="border-b border-[#1c2e1e] bg-[#0d1210]">
        <div className="max-w-4xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="text-xs text-[#6b7280]">Swing Trades</div>
        </div>
      </header>
